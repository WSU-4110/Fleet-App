    ...
    init() {
        authStateListener = Auth.auth().addStateDidChangeListener { [weak self] _, user in
            DispatchQueue.main.async {
                self?.user = user
                if let uid = user?.uid {
                    // Only restore as admin if last role was admin
                    if UserDefaults.standard.string(forKey: "lastRole") == "admin" {
    ...
    func signIn(username: String, password: String) {
        ...
        db.collection("usernameIndex").document(username.lowercased()).getDocument { [weak self] doc, error in
            ...
            Auth.auth().signIn(withEmail: email, password: password) { result, error in
                DispatchQueue.main.async {
                    if result != nil { UserDefaults.standard.set("admin", forKey: "lastRole") }
                    if error != nil {
